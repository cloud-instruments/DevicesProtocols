...
: RUN MACCOR SERVER
SET cmd=..\Release\ciupServer.exe -1 -p -f %logFilter% -l log\%dt%-ciupServer-MACCOR.log %port%
echo Executing %cmd%
start %cmd%
...
: RUN C++ CLIENT TEST 
SET cmd=..\Release\ciupClientTest.exe -f %logFilter% -l log\%dt%-ciupClientCpp-MACCOR.log %ip% %port%
echo Executing %cmd%
start %cmd%

: RUN MACCOR EMULATOR
SET cmd=..\Release\ciupServerMaccorDllTest.exe -c 0 %sleep%
echo Executing %cmd%
start %cmd% 
